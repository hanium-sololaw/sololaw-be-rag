...
> 상태: 문서 생성(`app/documents/`)·판례 검색(`app/cases/`, 하이브리드 RAG 포함) 도메인 구현·배포 완료, CI/CD 운영 중. **문서 생성 4종과 판례 검색 모두 화면 대조·검증 완료.** 증거 분석 도메인은 미착수. **인증(JWT) 미구현 — 최우선 과제.** `(TBD)` 항목은 변경될 수 있음.
...
- 준비서면(`brief`)은 **쟁점 단위 구조**다 — `RebuttalPoint`(상대 주장·반박·근거 증거·인용 판례) 하나가 문서의 "가.", "나." 항목이 된다. 상대 서면 종류·도달일로 "2026. 8. 7.자 준비서면(1)에서" 를 특정하고, `defenses`(항변 종류)·`undisputed_facts`(다툼 없는 사실)로 쟁점을 정리한다.
- 증거목록(`evidence_list`)은 민사소송규칙 제105조 제1항이 요구하는 **제목·작성자·작성일**에 입증취지와 원본/사본 구별을 더한 표(6열)다. 법에서는 「증거설명서」라 부른다.
- **미반영 필드 2건** (화면에는 있는데 스키마에 없음, 2026-08-19 확인):
  · 준비서면 `대리인` — 1단계의 「대리인을 선임했나요?」 접힌 섹션. 텍스트 한 칸(`변호사 ○○○`)이고 선택. 화면 안내상 준비서면 기재사항이라 당사자 표시 아래에 들어간다. `BriefInput` 에 `agent: str | None` 추가 + 프롬프트의 사건 섹션 규칙 한 줄.
  · 증거목록 `재판부` — 2단계의 「재판부 (선택)」. 준비서면에는 `panel` 이 있는데 증거목록에만 빠졌다. `EvidenceListInput` 에 `panel: str | None` 추가 + 관할법원을 "○○법원 ○○재판부 귀중" 으로.
  나머지는 화면과 대조 일치 확인 완료 (소장 5종 위저드, 신청서 5종, 준비서면 4단계, 증거목록 3단계).
