...
> 상태: 문서 생성(`app/documents/`)·판례 검색(`app/cases/`, 하이브리드 RAG 포함) 도메인 구현·배포 완료, CI/CD 운영 중. **피그마 화면 대조는 판례 검색 전체와 문서 생성 중 소장까지 완료** — 준비서면·증거목록·신청서는 미대조. 증거 분석 도메인은 미착수. **인증(JWT) 미구현 — 최우선 과제.** `(TBD)` 항목은 변경될 수 있음.
...
- **화면 대조는 소장만 끝났다.** 4종 API 는 모두 살아 있으나 준비서면·증거목록·신청서는 입력 필드가 피그마와 다를 수 있다. 특히 **신청서 종류가 어긋난다** — 코드는 기일변경·문서송부촉탁·보정서·소송구조 4종인데 화면은 지급명령·소송구조·임차권등기명령·강제집행·가압류 5종으로, 소송구조 하나만 겹치고 종류마다 필요한 입력이 서로 다르다.
- 소장(`complaint`)은 피그마 위저드(자가진단 → 정보입력 1~6단계)에 맞춰 정비 완료. 소송 유형 5종(`deposit_return`·`loan_return`·`wage_claim`·`damages`·`building_surrender`)이 화면과 1:1.
...
- 청구취지는 **금전 청구와 인도 청구로 분기**한다. 건물명도(`building_surrender`)만 1항이 "별지 목록 기재 건물을 인도하라"이고 미납 차임은 2항 금전 청구로 잇는다. 지연손해금 기산일은 **변제기 > 최고일(`demand_date`) > 소장 송달** 순으로 판단한다 — 우선순위를 프롬프트에 명시하지 않으면 LLM 이 최고일을 먼저 집는다.
